...
def check_pool(report: dict) -> None:
    counts = gender_counts(report["operational_voice_pool"])
    # O endpoint edge-tts atualmente expõe apenas Antonio como voz masculina pt-BR
    # operacional no runner. Exigimos cobertura de gênero real, sem cross-gender,
    # e duas vozes femininas; personagens masculinos adicionais usam personas
    # prosódicas distintas sobre a voz masculina disponível.
    assert counts["M"] >= 1 and counts["F"] >= 2, counts
    ...
    for e in report["episodes"]:
        ...
        if e["episode"] in expected_multi:
            ...
            if len(non_narrators) >= 2:
                assert identities, e
                assert len({identities[s] for s in non_narrators}) == len(non_narrators), e
            # Multivoz pode conter duas personas da mesma voz-base masculina;
